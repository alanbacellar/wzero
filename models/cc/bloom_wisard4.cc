...
public:
    ...
    BloomRam4(int tuple_lenght, int* mapping, int num_keys, int filter_tuple_lenght, int num_output) {
        ...
        std::mt19937 gen(rd());
        std::uniform_int_distribution<uint64_t> distrib(0, (uint32_t)0xFFFFFFFF);

        this->filter_keys = new uint64_t[tuple_lenght];
        for(int n = 0; n < this->tuple_lenght; ++n) {
            this->filter_keys[n] = distrib(gen);
            this->filter_keys[n] |= this->filter_keys[n] << 32;
        };
    ...
    uint64_t get_addr(ArrayND<bool>& input, int i) {
        uint64_t addr = ((input(i, this->mapping[0]) * (uint64_t)0xFFFFFFFF) | (input(i+1, this->mapping[0]) * ((uint64_t)0XFFFFFFFF00000000))) & this->filter_keys[0];
        for(int n = 1; n < this->tuple_lenght; ++n)
            addr ^= ((input(i, this->mapping[n]) * (uint64_t)0xFFFFFFFF) | (input(i+1, this->mapping[n]) * ((uint64_t)0XFFFFFFFF00000000))) & this->filter_keys[n];
        return addr;
    };
    ...
    void predict(ArrayND<bool>& input, ArrayND<atomic<int>>& output, int i) {
        ...
        key = keys & (uint64_t)0xFFFFFFFF;
        for(int o = 0; o < this->num_output; ++o)
            this->output[o] = 1;
        for(int k = 0; k < this->num_keys; ++k) {
            addr = (key >> (16 * k)) & this->filter_mask;
            for(int o = 0; o < this->num_output; ++o)
        ...
        key = (keys >> 32) & (uint64_t)0xFFFFFFFF;
        for(int o = 0; o < this->num_output; ++o)
            this->output[o] = 1;
        for(int k = 0; k < this->num_keys; ++k) {
            addr = (key >> (16 * k)) & this->filter_mask;
...
public:
    ...
    BloomWiSARD4(int input_lenght, int tuple_lenght, int num_keys, int filter_tuple_lenght, int num_classes) {
        ...
        this->rams = new BloomRam4*[this->num_rams];
        
        for(int i = 0; i < this->num_rams; ++i) {
            int ram_tuple_lenght = ((i == this->num_rams - 1) && offset ? (offset) : (tuple_lenght));
            this->rams[i] = new BloomRam4(ram_tuple_lenght, mapping[i], num_keys, filter_tuple_lenght, num_classes);
        };
    ...
    static void train_work(int thread, BloomWiSARD4* obj, ArrayND<bool>& input, ArrayND<int>& classes) {
        int ram;
    ...
    void train(ArrayND<bool>& input, ArrayND<int>& classes) {
        this->run_parallel(BloomWiSARD4::train_work, ref(input), ref(classes));
    };

    static void predict_work(int thread, BloomWiSARD4* obj, ArrayND<bool>& input, ArrayND<atomic<int>>& output, int begin, int end) {
        int ram;
    ...
    ArrayND<atomic<int>> predict(ArrayND<bool>& input, int begin=0, int end=-1) {
        ...
        this->run_parallel(BloomWiSARD4::predict_work, ref(input), ref(output), begin, end);
        return output;
    };
